...
{
    //Verifica que el recurso device esté correcto o bine creado
    if (device.m_device == nullptr) {
        ERROR("ShaderProgram", "init", "CHECK FOR Device device");
        exit(1);
    }
    //Verifica que el vector sea vàlido
                                 ...
                                 ShaderType type)
{
    ...
    hr = CompileShaderFromFile(m_shaderFileName.data(),
                               ...
                               shaderModel,
                               &shaderData);
    //Si el shader no se comñila bien manda error o no se asigna bien el shaderEntryPoint o el shaderModel
    if (FAILED(hr)) {
    ...
    // Create the shader segúin el dato asignado
