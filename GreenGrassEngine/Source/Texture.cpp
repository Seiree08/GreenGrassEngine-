...
HRESULT Texture::init(Device device, std::string textureName, ExtensionType extensionType)
{
	...
	case PNG:
		...
		//Cargar la imagen
		unsigned char* data = stbi_load(textureName.c_str(), &width, &height, &channels, 4); //4 para RGBA
		if (!data)
		{
			ERROR("Texture", "stbi_load", "Error al cargar la imagen: " << stbi_failure_reason())
			return E_FAIL;
		}
		//Descripción de la textura, la que se carga desde el formato se asigna en una dirección de memoria pa' luego cargarla
...
{
	//Validar que el  device funcione
	if (device.m_device == nullptr)
	{
		ERROR("Texture", "init", "CHECK FOR Device device on texture loading method")
			exit(1);
	}
	//Validación para el ancho y alto
	...
	desc.BindFlags = BindFlags; //Declara los recursos que vienen de los parámetros
	desc.CPUAccessFlags = 0;
	desc.MiscFlags = 0;

	hr = device.CreateTexture2D(&desc, nullptr, &m_texture);;
	//Revisa si la textura se genera correctamente, el contenido
	if (m_texture == nullptr)
	{
		ERROR("Texture", "init", "Error in data from params [CHECK FOR m_texture]")
			exit(1);
	}
	//Revisa la creación del recurso, la de la varible 
	else if (FAILED(hr))
	{
	...
	unsigned int NumViews)
{
	if (m_textureFromImg != nullptr)
	{
		...
		//Libera los recursos utilizados anteriormente
